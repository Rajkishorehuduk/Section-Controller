...
export const createDecision: RequestHandler = async (req, res) => {
  ...
  const base: Omit<Decision, "id"> = {
    ...
    acknowledgements: body.targets.reduce<
      Record<string, { acknowledged: boolean; at: string | null }>
    >((acc, station) => {
...
export const acknowledgeDecision: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (!data.acknowledgements || !data.acknowledgements[body.station])
      return res
        .status(400)
        .json({ error: "Station not targeted for this decision" });
    data.acknowledgements[body.station] = {
      acknowledged: true,
    ...
    res
      .status(500)
      .json({
        error: "acknowledgeDecision failed",
        message: e?.message || String(e),
      });
  }
};
